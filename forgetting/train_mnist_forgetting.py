...
parser.add_argument('-name', type=str, help='project name', default='mnist_forgetting')
...
def train(model, train_loader, optimizer, epoch, device, train_loss_lst, train_acc_lst, sample_forgetting,
          sample_acc_flag):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_loader):
        ...
        for i in range(inputs.size(0)):
            if sample_acc_flag[batch_idx * args.batch_size + i] == 1:  # last time right
                if pred[i] != labels[i]:  # this time wrong
                    sample_forgetting[batch_idx * args.batch_size + i] += 1
            sample_acc_flag[batch_idx * args.batch_size + i] = 1 if pred[i] == labels[i] else 0
    ...
    return train_loss_lst, train_acc_lst, sample_forgetting, sample_acc_flag
...
if __name__ == "__main__":
    ...
    sample_forgetting = np.zeros((len(train_set)))  # each sample's forgetting time
    sample_acc_flag = np.zeros((len(train_set)))  # last time pred right(1) or wrong(0)

    # main loop(train,val,test)
    for epoch in range(args.epochs):
        train_loss_lst, train_acc_lst, sample_forgetting, sample_acc_flag = train(model, train_loader, optimizer,
                                                                                  epoch, device, train_loss_lst,
                                                                                  train_acc_lst,
                                                                                  sample_forgetting, sample_acc_flag)
