...
from truncated_loss import QLoss
from utils.dataset import MNISTNoisy, CIFAR10Noisy, CIFAR100Noisy

parser = argparse.ArgumentParser()
parser.add_argument('-project_name', type=str, help='project name', default='mnist_noisy_qloss_s0.6')
...
def train(model, train_loader, optimizer, epoch, device, train_loss_lst, train_acc_lst, sample_loss):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_loader):
        ...
        # record loss
        criterion = QLoss(reduction='none')
...
if __name__ == "__main__":
    ...
    plt.fill_between(range(args.epochs), clean_min_loss_lst, clean_max_loss_lst, color='aquamarine', alpha=0.5)
    plt.fill_between(range(args.epochs), noisy_min_loss_lst, noisy_max_loss_lst, color='paleturquoise', alpha=0.5)
